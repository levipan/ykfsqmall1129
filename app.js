...
App({
  ...
  onLaunch: function () {
    ...
    //云开发初始化
    wx.cloud.init({
      env: 'prod-924a3b', //2-src替换成你自己的云开发环境id
      traceUser: true,
    })
    this.getOpenid();
  },
  ...
  //保存选择的团长
  _saveTuanZhang(tuanzhang) {
    this.globalData.tuanzhang = tuanzhang
    wx.setStorageSync('tuanzhang', tuanzhang)
  },
  //获取本地保存的团长
  _getTuanZhang() {
    let tz = wx.getStorageSync('tuanzhang')
    if (tz && tz.name) {
      this.globalData.tuanzhang = tz
    }
  },
